...
class Command(BaseCommand):
    help = "Send web push notifications for normal classes starting in 5 minutes."
    ...
    def handle(self, *args, **options):
        ...
        day_code = weekday_codes[target.weekday()]
        ...
        # Normal lessons
        if slots.exists():
            self.stdout.write(f"Processing {slots.count()} normal slots starting in 5 minutes:")
        ...
        if remedial_tts.exists():
            self.stdout.write(f"Processing {remedial_tts.count()} remedial slots starting in 5 minutes:")
            ...
            for tt in remedial_tts:
                ...
                payload_r = json.dumps({
                    "title": "Next remedial class in 5 minutes",
        ...
        if not slots.exists() and not remedial_tts.exists():
            self.stdout.write("No normal or remedial classes starting in 5 minutes.")
